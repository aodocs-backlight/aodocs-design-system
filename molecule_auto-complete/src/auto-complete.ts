...
import { bgcColorMixin, color } from '../../style_colors';
import { absolute, fullWidth, zIndexMixin } from '../../style_positioning';
import '../../textfield';
...
export class AutoComplete extends LitElement {
  ...
  public render() {
    return html`
      <div style="position: relative">
        ...
        ></mwc-textfield>
        ${this.dislayLoader()} ${this._displayAutoCompleteList()}
      </div>
    `;
  }
  ...
  private _displayAutoCompleteList(): TemplateResult {
    ...
    return html`
      <mwc-list
        activatable
        id="activatable"
        class="elevation absolute bgc--white z-index-1000 full-width"
        @click
        style="width: 100%"
      >
  ...
  private _listItems(items: string[] | Nested[]): TemplateResult[] {
    if (!this._canDisplayItemsList) {
      ...
      return [
        html`
          <mwc-list-item><h4>${this.noData}</h4></mwc-list-item>
        `
      ];
    ...
    const values = (<string[]>items).filter(item =>
      item.includes(this._search ?? '')
    );
    return values?.length > 0
      ? this._stringListItems(values)
      : [
          html`
            <mwc-list-item><h4>${this.noData}</h4></mwc-list-item>
          `
    ...
    return items?.map(
      item =>
        html`
          <mwc-list-item
            value="${item}"
            @click=${() => this._setSelectedValue(item)}
          >
            ${item}
          </mwc-list-item>
        `
  ...
  private _checkReturnObjectRequirements() {
    if (!this.displayName) {
      throw new Error(
        `In order to use the aodocs-autocomplete, you need to provide the display name object key`
      );
    }
  }
  ...
  private get _debounce(): number {
    return (
      this.debounce ?? this.DEBOUNCE_TIME * this.DEBOUNCE_TIME_CONVERTER_MS
    );
  }
